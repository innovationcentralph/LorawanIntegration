...
#define QUEUE_SIZE 20  // The size of the rolling queue

typedef struct {
    uint16_t waterLevel;      // Scaled by 100 (e.g., 123.456 -> 12345)
    uint16_t batteryVoltage;  // Scaled by 100 (e.g., 234.567 -> 23456)
    uint8_t systemState;
} CommandData;

typedef struct {
    CommandData buffer[QUEUE_SIZE];  // Queue buffer to store the elements
    uint8_t head;  // Index of the head (newest element)
    uint8_t tail;  // Index of the tail (oldest element)
    uint8_t count; // Number of elements in the queue
} RollingQueue;

extern RollingQueue commandQueue;
/* USER CODE END EM */
